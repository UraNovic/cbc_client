...
describe('ExchangeCtrl', function() {
  ...
  it('should be initialized with defaults', function(done) {
    ...
    assert.strictEqual(scope.cbc.code, 'cbc');
    assert.isObject(scope.cbc.currency);
    done();
  });
  ...
  it('should update currency_choices after setting trust lines', function(done) {
    ...
    assert.strictEqual(scope.currency_choices[0], 'cbc - cbcs');

    done();
  });

  it('should update paths after entering 2 as amount for cbc', function(done) {
    ...
    setTimeout(function() {
      ...
      assert.strictEqual(scope.exchange.amount_feedback && scope.exchange.amount_feedback.to_text_full(), '2/cbc', 'name wrong');
