...
describe('SendCtrl', function(){
  ...
  beforeEach(inject(function($rootScope, $controller, rpNetwork) {
    ...
    scope.currencies_all = [{ name: 'cbcs', value: 'cbc'}];
  ...
  it('should be initialized with defaults', function (done) {
    assert.isObject(scope.cbc);
    assert.strictEqual(scope.cbc.name, 'cbc - cbcs');
    assert.isObject(scope.send);
    assert.strictEqual(scope.send.currency, 'cbc - cbcs');
    assert.strictEqual(scope.send.currency_code, 'cbc');
  ...
  describe('setting a receiver', function() {
    ...
    it("should update the currency_choices with the receiver's options", function(done) {
      ...
      scope.send.currency_choices_constraints.accountLines = ["cbc", "BEE", "BRL", "DPD", "FLO", "FRI", "GUM", "LTC", "NMC", "PPC", "SFO", "TDO", "TIK", "TIM", "USD", "XAU", "015841551A748AD2C1F76FF6ECB0CCCD00000000"];
      ...
      assert.isTrue(_.contains(scope.send.currency_choices, "cbc - cbcs"));
    ...
    it("should intersect currency_choices from federation with the receiver's options", function(done) {
      scope.send.currency_choices_constraints = {};
      scope.send.currency_choices_constraints.accountLines = ["cbc", "BEE", "BRL", "DPD", "FLO", "FRI", "GUM", "LTC", "NMC", "PPC", "SFO", "TDO", "TIK", "TIM", "USD", "XAU", "015841551A748AD2C1F76FF6ECB0CCCD00000000"];
      scope.send.currency_choices_constraints.federation = ["BRL", "SFO", "015841551A748AD2C1F76FF6ECB0CCCD00000000", "XXX", "DOG", "cbc"];
      scope.update_currency_choices();
    ...
    it.skip("should reset if the recipient is not valid", function(done) {
      ...
      assert.strictEqual(scope.send.currency_choices[0], 'cbc - cbcs');
