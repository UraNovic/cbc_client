  ...
  this.inboundBridgeProfile = function(manifest) {
    return {
      ...
      trust: function(currency,issuer) {
        ...
        tx.cbcLineSet(id.account, line.amount + '/' + line.currency + '/' + line.issuer);
        tx.setFlags('Nocbc');
       ...
       * @param callback
       */
      getInstructions: function(cbcAddress, callback) {
        $http({
          url: manifest.urls.instructions,
          method: 'GET',
          params: {cbcAddress: cbcAddress}
        })
       ...
       * @param callback
       */
      getPending: function(cbcAddress, callback) {
        $http({
