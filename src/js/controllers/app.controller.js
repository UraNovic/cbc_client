                  ...
                  $timeout, rpHistory, $templateRequest)
{
  ...
  var removeBlobUpdateListener = $scope.$watch('userBlob', function() {
    if ($scope.userBlob.data && $scope.userCredentials.username) {
      if (settings.getSetting($scope.userBlob, 'showDisclosure') !== false) {
        $scope.userBlob.set('/clients/cbctradecom/showDisclosure', true);
      }
  ...
  $scope.dismissBanner = function() {
    $scope.userBlob.set('/clients/cbctradecom/showDisclosure', false);
    $scope.showTerms = settings.getSetting($scope.userBlob, 'showDisclosure');
  };
  ...
  if ("object" === typeof cbcclient) {
    cbcclient.id = id;
    cbcclient.net = net;
    cbcclient.keychain = keychain;
    // for unit tests
    cbcclient.settingsUtils = settings;
  }
  ...
  function handleAccountLoad(e, data)
  {
    ...
    remote.requestAccountLines({account: data.account})
      .on('success', handlecbcLines)
      .on('error', handlecbcLinesError).request();
    ...
    $scope.$apply(function () {
      ...
      for (var n = 0, l = data.lines.length; n < l; n++) {
        ...
        if ($scope.isSignificantLine(line)) {
          ...
          //      library upstream.
          line = $.extend({}, line, {
            limit: cbc.Amount.from_json({value: line.limit, currency: line.currency, issuer: line.account}),
            limit_peer: cbc.Amount.from_json({value: line.limit_peer, currency: line.currency, issuer: line.account}),
            balance: cbc.Amount.from_json({value: line.balance, currency: line.currency, issuer: line.account})
          });

          $scope.lines[line.account + line.currency] = line;

          // TEMPORARY: Check if user has trust to cbcTradeJapan to display banner
          ...
          if (line.account === 'rMAz5ZnK73nyNUL4foAvaxdreczCkG3vA6') {
            ...
            if (!line.no_cbc || !line.no_cbc_peer) {
              $scope.hasRTJapanTrustWithRippling = true;
            }
          }

          updatecbcBalance(line.currency, line.account, line.balance);
        }
      }
  ...
  function handlecbcLinesError(data)
  {
    $scope.$apply(function () {
      $scope.loadState.lines = true;
    });
  ...
  function handleOffers(data)
  {
    $scope.$apply(function () {
      data.offers.forEach(function (offerData) {
        var offer = {
          seq: +offerData.seq,
          gets: cbc.Amount.from_json(offerData.taker_gets),
          pays: cbc.Amount.from_json(offerData.taker_pays),
  ...
  {
    if (offer.flags && offer.flags === cbc.Remote.flags.offer.Sell) {
      offer.type = 'sell';
  ...
  function updateLines(effects)
  {
    ...
    $.each(effects, function () {
          ...
          'trust_change_no_cbc'], this.type))
      {
        ...
        line.no_cbc = !!effect.nocbc; // Force Boolean

        if (effect.balance) {
          line.balance = effect.balance;
          updatecbcBalance(effect.currency,
            effect.counterparty,
            effect.balance);
  ...
  function updatecbcBalance(currency, new_account, new_balance)
  {
  ...
  if (!store.disabled && !store.get('cbc_pairs_all')) {
    store.set('cbc_pairs_all',require('../data/pairs'));
  }

  var pairs_all = store.get('cbc_pairs_all');
  ...
  var currenciesAllWatcher = $scope.$watch('currencies_all', function(){
    ...
    if (!store.disabled) {
      store.set('cbc_currencies_all',$scope.currencies_all);
    }
  ...
  var pairsAllWatcher = $scope.$watch('pairs_all', function(){
    ...
    if (!store.disabled) {
      store.set('cbc_pairs_all',$scope.pairs_all);
    }
  ...
  //      lost and later re-established. (... or will the cbc lib do that for us?)
  ...
  $scope.generate_src_currencies = function () {
    ...
    src_currencies.push({ currency: "cbc" });
  ...
  this.handlecbcLines      =  handlecbcLines;
  this.handlecbcLinesError =  handlecbcLinesError;
  ...
  this.updatecbcBalance    =  updatecbcBalance;
