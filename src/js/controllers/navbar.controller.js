                                           ...
                                           network, $location)
{
  ...
  $scope.$watch('connected', function(){
    ...
    // $scope.currentFee = cbc.Amount.from_json($scope.fee).to_human(); 

    if (!$scope.connected && $scope.userCredentials.username) {
      $scope.serverStatus = 'disconnected';
    }
    else if ($scope.connected && $scope.fee) {
      if ((parseFloat(cbc.Amount.from_json($scope.fee).to_human()) > parseFloat(Options.low_load_threshold)) && (parseFloat($scope.fee) < parseFloat(Options.max_tx_network_fee))) {
  ...
  $scope.read = function() {
    ...
    if (settings.getSetting($scope.userBlob, 'lastSeenTxDate', 0) < lastTxDate) {
      ...
      $scope.userBlob.set('/clients/cbctradecom/lastSeenTxDate', lastTxDate);
