    ...
    link: function (scope, elm, attr, ctrl) {
      ...
      ctrl.$asyncValidators.rpMasterAddressExists = function(value) {
        if (!value || !Base.decode_check(33, value)) {
          ...
          var accountId = cbc.Seed.from_json(value).get_key().get_address().to_json(),
              defer = $q.defer();

          $timeout(function() {
            cbcVaultClient.AuthInfo.get(Options.domain, accountId, function(err, data) {
              if (err) {
                defer.reject();
              }
 ...
 *
 * - rp-dest-address          - If set, allows cbc addresses as destinations.
 ...
 * - rp-dest-cbc-name      - If set, allows Existing cbc name as destination.
 * - rp-dest-model            - If set, updates the model with the resolved cbc address.
 *
  ...
  return {
    ...
    link: function (scope, elm, attr, ctrl) {
      ...
      ctrl.$asyncValidators.rpDest = function(value) {
        var strippedValue = webutil.stripcbcAddress(value),
            address = cbc.UInt160.from_json(strippedValue);

        if (typeof strippedValue !== 'string'
          || (strippedValue.length && strippedValue[0] !== 'r')) {
            address = cbc.UInt160.from_json();
        }
        ...
        if (((attr.rpDestcbcName && webutil.iscbcName(value)) ||
          (attr.rpDestcbcNameNoTilde && value && value[0] !== '~' && webutil.iscbcName('~' + value)))) { // TODO Don't do a client check in validators
          ctrl.rpDestType = "cbcName";
          ...
          currentCheckTimeout = $timeout(function() {
            ...
            cbcVaultClient.AuthInfo.get(Options.domain, value, function(err, info) {
...
/**
 * Check if the cbc name is valid and is available for use
 */
  ...
  return {
    ...
    link: function (scope, elm, attr, ctrl) {
      ...
      var validator = function(value) {
        ...
        if ( (shortValue==="cbc") || webutil.findIssuer(scope.lines,shortValue))
        {
...
module.directive('rpAmountcbcLimit', function () {
  return {
    restrict: 'A',
    require: '?ngModel',
    link: function (scope, elm, attr, ctrl) {
      ...
      var validator = function(value) {
        ...
        if (currency !== 'cbc') {
          ctrl.$setValidity('rpAmountcbcLimit', true);
        } else {
          ctrl.$setValidity('rpAmountcbcLimit', value <= 100000000000 && value >= 0.000001);
        }
 ...
 */
module.directive('rpNotcbc', function () {
  return {
    restrict: 'A',
    require: '?ngModel',
    link: function (scope, elm, attr, ctrl) {
      ...
      var validator = function(value) {
        ctrl.$setValidity('rpNotcbc', !value || value.toLowerCase() !== 'cbc');
        return value;
      };
      ...
      attr.$observe('rpNotcbc', function() {
        validator(ctrl.$viewValue);
      });
    }
  };
