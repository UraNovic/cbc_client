...
(function() {
  ...
  /* global cbc: false, angular: false, _: false, jQuery: false, store: false, Options: false */
  ...
  AddressPopover.prototype.makeContent = function() {
    ...
    if (this.cbcName) {
      if (this.cbcName !== this.identity) {
        htmlContent.push('<div>', this.cbcName, '</div>');
      }
      htmlContent.push('<span>', this.identity, '</span>', '<br/>');
      htmlContent.push('<a target="_blank", href="http://www.cbccharts.com/#/graph/', this.identity, '" >', AddressPopover.textShowInGraph, '</a>');
    } else if (this.identity) {
      htmlContent.push(AddressPopover.textcbcAddress, ' ', this.identity);
    }
  ...
  AddressPopover.textcbcAddress = 'span(l10n) cbc address';
  AddressPopover.textShowInGraph = 'span(l10n) Show in graph';

  /**
   * Special popover to show cbc address with ability to double click on address to select.
   * Also can link to www.cbccharts.com.
   * rp-address-popover-link-to-charts - show ling to cbc charts
  ...
  function rpAddressPopover($timeout, id, $filter, $parse, $templateRequest) {
    $templateRequest('templates/' + window.lang + '/directives/addresspopover.html', false).then(function(template) {
      ...
      AddressPopover.textcbcAddress = texts[0];
