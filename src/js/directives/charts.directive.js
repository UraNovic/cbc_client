...
module.directive('rpPieChart', ['$filter', function($filter) {
  ...
  function contactButNotcbc(x) {
    return x === 'cbc' ? 'cbc' : rpcontactname(x);
  }
  ...
  function pieChart(element, scope) {
    ...
    function drawPieChart(container, exchangeRates, drops, ious) {
      ...
      var cbcAsSuch = parseInt(drops, 10) / 1000000;
      ...
      for (var cur in ious) {if (ious.hasOwnProperty(cur)){
        ...
        for (issuer in components) {if (components.hasOwnProperty(issuer)){
          ...
          if (amount > 0) {
            var subbalanceAscbc = amount * (exchangeRates[cur + ':' + issuer] || 0);
            totalAscbc += subbalanceAscbc;
            issuerSubshares.push({
              issuer: contactButNotcbc(issuer),
              subbalance: subbalanceAscbc
            });
          }
        }}
      ...
      // -Divide all the subbalances by totalAscbc, inserting the result as 'subshare's
      ...
      for (i = 0; i < pieces.length; i++) {
        ...
        for (j = 0; j < piece.issuerSubshares.length; j++) {
          ...
          is.subshare = is.subbalance / totalAscbc;
          share += is.subshare;
        }
        piece.share = share;
      ...
      $('<circle></circle>').appendTo(container.find('svg')).attr({
        ...
        cx:   SIZE / 2,
        cy:   SIZE / 2,
        r:    SIZE * 2 / 9 // like on cbccharts.com
      });
