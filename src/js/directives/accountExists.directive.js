...
/* global cbc: false, angular: false, _: false, jQuery: false, store: false, Options: false */
   ...
   *
   * Checks with cbcd network if account exists.
   * According to cbcd unfunded accounts do not exist.
   */
        ...
        ctrl.$asyncValidators.rpAccountExists = function(value) {
          ...
          function checkFunded(addressToCheckPromise) {
            return addressToCheckPromise.then(function(addressToCheck) {
              ...
              var address2 = cbc.UInt160.from_json(addressToCheck);
          ...
          } else if (webutil.iscbcName(value)) {
            var defer = $q.defer();
            currentDefer = defer;
