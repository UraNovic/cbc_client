...
require('../services/cbctxt.service.js');
...
var tabdefs = [
  ...
  require('../tabs/cbc.controller.js'),
...
window.lang = (function(){
  ...
  return resolveLanguage(store.get('cbc_language')) ||
...
var cbcclient = window.cbcclient = {};
cbcclient.app = app;
cbcclient.types = types;
// for unit tests
cbcclient.rewriter = rewriter;

// for unit tests
cbcclient.tabs = {};
_.each(tabs, function(tab) { cbcclient.tabs[tab.tabName] = tab; });
...
function Config ($routeProvider, $injector) {
  ...
  $routeProvider.when('/lang/:language', {
    redirectTo: function(routeParams, path, search){
      ...
      if (!store.disabled) {
        store.set('cbc_language', lang ? lang : '');
      }
...
{
  $rootScope.productName = 'cbc Trade';

  // Global reference for debugging only (!)
  if ("object" === typeof cbcclient) {
    cbcclient.$scope = $rootScope;
    cbcclient.version = $rootScope.version =
      angular.element('#version').html();
    if (typeof debug !== "undefined" && debug === true) {
      cbcclient.versionBranch = $rootScope.versionBranch =
