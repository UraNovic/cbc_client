...
module.filter('rpamount', function() {
  return function (input, options) {
    ...
    if (opts.cbc_human && input === ("" + parseInt(input, 10))) {
      input += "/cbc";
    }
    ...
    if (!opts.hard_precision && input._is_native) {
      // If cbc, then set standard precision here
 ...
 * Show cbc Name
 *
 * Shows a cbc name for a given cbc address
 */
module.filter("rpcbcname", ['rpId', function(id) {
  return function(address, options) {
    var cbcname = id.resolveNameSync(address, options);
    if (cbcname !== address) {
      return cbcname;
    }
...
module.filter("rpcbcnamefull", ['rpId', function(id) {
  return function(address, options) {
    var cbcname = id.resolveNameSync(address, options);
    if (cbcname !== address) {
      return cbcname;
...
module.filter('rpaddressorigin', function() {
  return function(recipient) {
    ...
    return !isNaN(Base.decode_check([0, 5], recipient, 'bitcoin')) ? 'bitcoin' : 'cbc';
  };
});
    ...
    arrayCopy.sort(function(a, b) {
      ...
      switch(field) {
        case 'qty':
          ...
          // Amount.compareTo doesn't compare cbc and non-cbc
