...
SettingsGatewayTab.prototype.angular = function(module)
{
                                    ...
                                    function ($scope, id, keychain, network, api)
  {
    ...
    $scope.edit = {
      ...
      bridge: false,
      defaultcbcFlag: false,
      defaultcbcFlagSaving: false
    };
    ...
    $scope.save = function(type) {
      switch (type) {
        case 'advanced_feature_switch':
          $scope.userBlob.set('/clients/cbctradecom/trust/advancedMode', $scope.options.advanced_feature_switch);
          break;
        case 'defaultcbcFlag':
          // Need to set flag on account_root only when chosen option is different from current setting
          if ($scope.currentDefaultRipplingFlagSetting !== $scope.isDefaultcbcFlagEnabled) {
            $scope.edit.defaultcbcFlagSaving = true;
            var tx = network.remote.transaction();
            !$scope.isDefaultcbcFlagEnabled ? tx.accountSet(id.account, undefined, 'Defaultcbc') : tx.accountSet(id.account, 'Defaultcbc');
            tx.on('success', function(res) {
              $scope.$apply(function() {
                $scope.edit.defaultcbcFlagSaving = false;
                $scope.load_notification('defaultcbcUpdated');
              });
            ...
            keychain.requestSecret(id.account, id.username, function (err, secret) {
              if (err) {
                ...
                $scope.isDefaultcbcFlagEnabled = !$scope.isDefaultcbcFlagEnabled;
                $scope.edit.defaultcbcFlagSaving = false;
                return;
              }
              tx.secret(secret);
    ...
    $scope.$watch('account', function() {
      ...
      $scope.isDefaultcbcFlagEnabled = !!($scope.account.Flags & cbc.Remote.flags.account_root.Defaultcbc);
      $scope.currentDefaultRipplingFlagSetting = $scope.isDefaultcbcFlagEnabled;
    }, true);
