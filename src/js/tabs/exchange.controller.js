    ...
    function ($scope, $timeout, $routeParams, id, network, rpTracker, keychain, api, $rootScope, $location)
    {
      ...
      if (settings.blobIsValid($scope.userBlob)) {
        if (settings.getSetting($scope.userBlob, 'cbcExchangeSelectionTrade', false)) {
          $scope.userBlob.set('/clients/cbctradecom/cbcExchangeSelectionTrade', false);
        }
      } else {
      ...
      $rootScope.cbc_exchange_selection_trade = false;
      ...
      $scope.$watch('exchange.currency_name', function() {
        ...
        var currency = Currency.from_human($scope.exchange.currency_name ? $scope.exchange.currency_name : 'cbc');
      ...
      $scope.update_exchange = function() {
        var exchange = $scope.exchange;
        var currency = cbc.Currency.from_human(exchange.currency_name);
      ...
      var updateCurrencyOptions = function() {
        ...
        // add cbc
        currencies.unshift('cbc');

        // create a currency object for each of the currency codes
        for (var i = 0; i < currencies.length; i++) {
          currencies[i] = cbc.Currency.from_json(currencies[i]);
        }
      ...
      $scope.reset = function () {
        ...
        $scope.exchange = {
          amount: '',
          currency_name: $scope.cbc.name,
          currency_code: $scope.cbc.code,
          currency_obj: $scope.cbc.currency,
          path_status: 'waiting',
          fund_status: 'none'
        };
        $scope.nickname = '';
