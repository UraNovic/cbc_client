...
TradeTab.prototype.angular = function(module)
{
                     ...
                     $timeout, $templateRequest)
  {
    ...
    function onBlobIsValid() {
      ...
      if (!settings.getSetting($scope.userBlob, 'cbcExchangeSelectionTrade', false)) {
        $scope.userBlob.set('/clients/cbctradecom/cbcExchangeSelectionTrade', true);
      }
    }
    ...
    // Remember user preference on Convert vs. Trade
    $rootScope.cbc_exchange_selection_trade = true;
    ...
    // Details for an order that is edited and sent to cbc for modification
    $scope.editOrder = {
      seq: null,
      type: null,
      oldPrice: null,
    ...
    // Format a cbc Amount for editing by the user
    ...
    // Create a cbc Amount from primitives
    function createAmount(value, currency, issuer) {
      var ccy = currency || Currency.from_json('cbc');
    ...
    $scope.reset = function () {
      ...
      var pair = store.get('cbc_trade_currency_pair') || $scope.pairs_all[0].name;
      ...
      else if(store.get('cbc_trade_listing')) {
        listing = store.get('cbc_trade_listing');
      }
      else {
        listing = 'orderbook';
      }
    ...
    function getOrderCurrency(entry) {
      ...
      var first = first_currency === 'cbc'
        ? 'cbc'
        : first_currency + '.' + first_issuer;

      var second = second_currency === 'cbc'
        ? 'cbc'
    ...
    $scope.goto_order_currency = function() {
      ...
      var first = order.first_currency === 'cbc'
        ? 'cbc'
        : order.first_currency + '.' + order.first_issuer;

      var second = order.second_currency === 'cbc'
        ? 'cbc'
      ...
      keychain.requestSecret(id.account, id.username, function (err, secret) {
        ...
        api.getUserAccess().then(function(res) {
          ...
          cancelOrder.errorMsg =
            'Sorry, users from your region are not allowed to do transactions through cbc Trade.';
          $rootScope.load_notification('cancel_error');
    ...
    // 2. cbcd receives the request and cancels the old order.
    ...
    $scope.update_first = function (type) {
      ...
      var first_currency = $scope.order.first_currency || Currency.from_json('cbc');
      var formatted = '' + order.first + ' ' + (first_currency.has_interest() ? first_currency.to_hex() : first_currency.get_iso());

      order.first_amount = cbc.Amount.from_human(formatted, {reference_date: new Date(+new Date() + REF_DATE_OFFSET)});
    ...
    $scope.update_price = function (type) {
      var order = $scope.order[type];
      var second_currency = $scope.order.second_currency || Currency.from_json('cbc');
      var formatted = '' + order.price + ' ' + (second_currency.has_interest() ? second_currency.to_hex() : second_currency.get_iso());

      order.price_amount = cbc.Amount.from_human(formatted, {reference_date: new Date(+new Date() + REF_DATE_OFFSET)});
    ...
    $scope.update_second = function (type) {
      ...
      order.second_amount = cbc.Amount.from_human(formatted, {reference_date: new Date(+new Date() + REF_DATE_OFFSET)});
    ...
    $scope.calc_first = function (type) {
      ...
      if (order.price_amount  && order.price_amount.is_valid() &&
        ...
        if (!order.price_amount.is_native()) {
          ...
          // use replace(/,/g,'') until cbc lib fixed
    ...
    $scope.$watch('first_currency_selected', function () {
      ...
      if($scope.first_currency_selected == 'cbc') {
    ...
    $scope.$watch('second_currency_selected', function () {
      ...
      if($scope.second_currency_selected == 'cbc') {
    ...
    $scope.add_pair = function() {
      var formattedIssuerFirst = $scope.first_currency_selected === 'cbc' ? '' : '.' + $scope.first_issuer_selected;
      var formattedIssuerSecond = $scope.second_currency_selected === 'cbc' ? '' : '.' + $scope.second_issuer_selected;
      if (($scope.second_currency_selected !== 'cbc' && ($scope.second_issuer_selected == null || $scope.second_issuer_selected === '')) ||
          ($scope.first_currency_selected  !== 'cbc' && ($scope.first_issuer_selected  == null || $scope.first_issuer_selected  === ''))) {
      ...
      $scope.userBlob.unshift('/clients/cbctradecom/trade_currency_pairs', { name: $scope.order.currency_pair });
    ...
    function updateSettings() {
      ...
      if (!store.disabled) {
        store.set('cbc_trade_currency_pair', pair);
      }
      ...
      if (!pair || pair.length != 2 || pair[0].length === 0 || pair[1].length === 0) {
        order.first_currency = Currency.from_json('cbc');
        order.second_currency = Currency.from_json('cbc');
        order.valid_settings = false;
        return;
      }
      
      var first_currency = order.first_currency = cbc.Currency.from_json(pair[0].substring(0,3));

      var second_currency = order.second_currency = cbc.Currency.from_json(pair[1].substring(0,3));
      ...
      if(first_currency.is_native()) {
        ...
        if (contact_to_address1) {
          ...
          if (pair[0].substring(4, 5) == '~') {
            cbcVaultClient.AuthInfo.get(Options.domain, pair[0].substring(4), function (err, response1) {
              if (err) return;
      ...
      if(second_currency.is_native()) {
        ...
        if (contact_to_address2) {
          ...
          if (pair[1].substring(4, 5) == '~') {
            cbcVaultClient.AuthInfo.get(Options.domain, pair[1].substring(4), function (err, response2) {
              if (err) return;
              $scope.$apply(function () {
      ...
      var first_issuer = cbc.UInt160.from_json(order.first_issuer);
      var second_issuer = cbc.UInt160.from_json(order.second_issuer);
      // Invalid issuers or cbc/cbc pair
    ...
    $scope.toggleOffers = function() {
      ...
      $scope.userBlob.set('/clients/cbctradecom/tradeoffers', {hideOffers: $scope.hideOffers, hideOrderBook: $scope.hideOrderBook});
    };
    ...
    $scope.$watchCollection('[userBlob.data.clients.cbctradecom.tradeoffers.hideOffers, userBlob.data.clients.cbctradecom.tradeoffers.hideOrderBook]', function(e){
    ...
    function update_pairs() {
      ...
      if (!settings.hasSetting($scope.userBlob, 'trade_currency_pairs')) {
        $scope.pairs_query = [{name: 'cbc/USD.SnapSwap' },
          { name: 'cbc/USD.Bitstamp' },
          { name: 'cbc/JPY.TokyoJPY' },
          { name: 'BTC.Bitstamp/cbc' },
          { name: 'BTC.SnapSwap/cbc' }];
      }
      else {
        $scope.pairs_query = settings.getSetting($scope.userBlob, 'trade_currency_pairs');
      }
    ...
    if ($routeParams.first && $routeParams.second) {
      ...
      ['first','second'].forEach(function(prefix){
        ...
        param_is_valid[prefix] = !!routeCurrencies[prefix] && (routeCurrencies[prefix][1] === 'cbc' || !!routeIssuers[prefix]);
      });

      if (param_is_valid.first && param_is_valid.second && routeCurrencies.first[1] !== routeCurrencies.second[1]) {
          currencyPairChangedByNonUser = true;
          var formattedIssuerFirst = routeCurrencies.first[1] === 'cbc' ? '' : '.' + routeIssuers.first[1];
          var formattedIssuerSecond = routeCurrencies.second[1] === 'cbc' ? '' : '.' + routeIssuers.second[1];
