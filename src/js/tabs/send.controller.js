...
SendTab.prototype.angular = function (module)
{
                                           ...
                                           keychain, api, $interval)
  {
    ...
    $scope.$watch('send.recipient', function(rec){
      ...
      var address = webutil.stripcbcAddress($scope.send.recipient);
    ...
    $scope.$watch('send.currency', function () {
      var currency = cbc.Currency.from_json($scope.send.currency);
    ...
    $scope.update_destination = function () {
      ...
      // Trying to send cbc to self.
      // This is used to disable 'Send cbc' button
      send.self = recipient === $scope.address;

      // Trying to send to a cbc name
      send.cbcName = webutil.iscbcName(recipient);
      ...
      if (send.federation) {
        ...
        else if (recipient.substring(recipient.indexOf('@')+1) == 'btc2cbc.com' &&
          store.get('profile_country') == 'US') {

          if ($scope.sendForm && $scope.sendForm.send_destination) {
            $scope.sendForm.send_destination.$setValidity('btc2cbcUSCustomer', false);
          }
          return;
        }
      }
    ...
    $scope.update_destination_remote = function () {
      var send = $scope.send;
      var recipient = send.recipient_address;
      var strippedRecipient = webutil.stripcbcAddress(send.recipient);
      var isRecipientValidAddress = cbc.UInt160.is_valid(strippedRecipient);
      ...
      else if (send.cbcName) {
        cbcVaultClient.AuthInfo.get(Options.domain, send.recipient, function(err, response) {
          $scope.$apply(function(){
    ...
    $scope.check_destination = function () {
      ...
      account.entry(function (e, data) {
        $scope.$apply(function () {
          ...
          // If we get this far, we have a cbc address resolved
          send.recipient_resolved = true;
          ...
          if (e) {
            ...
            send.recipient_info = {
              ...
              // Flags
              disallow_cbc: data.account_data.Flags & cbc.Remote.flags.account_root.Disallowcbc,
              dest_tag_required: data.account_data.Flags & cbc.Remote.flags.account_root.RequireDestTag
            };
            ...
            send.cbc_deficiency = reserve_base.subtract(data.account_data.Balance);

            if (!send.federation && $scope.contact &&
                !send.recipient_info.disallow_cbc &&
                !send.recipient_info.dest_tag_required &&
                send.cbc_deficiency.compareTo(0) <= 0) {
    ...
    $scope.update_currency_constraints = function () {
      ...
      // If we don't have information about the recipient cbc account yet,
      ...
      if (send.recipient_info.exists) {
          ...
          .on('success', function (data) {
            $scope.$apply(function () {
              if (data.receive_currencies &&
                ...
                // add cbc if it's allowed
                if (!$scope.send.recipient_info.disallow_cbc) {
                  send.currency_choices_constraints.accountLines.unshift('cbc');
                }
        ...
        // If the account doesn't exist, we can only send cbc
        send.currency_choices_constraints.accountLines = ['cbc'];
      }
      ...
      if (_.values(send.currency_choices_constraints).indexOf('pending') !== -1) {
        ...
        currencies = _.map(currencies, function (currency) {
         // create a currency object for each of the currency codes
          var currencyObj = cbc.Currency.from_json(currency);
    ...
    $scope.update_currency = function () {
      ...
      if (!cbc.UInt160.is_valid(recipient) && !send.quote_url) {
        return;
      }
    ...
    $scope.reset_amount_deps = function () {
      var send = $scope.send;
      send.sender_insufficient_cbc = false;
      send.quote = false;
    ...
    $scope.update_amount = function () {
      ...
      var currency = cbc.Currency.from_human(send.currency);
      ...
      var amount = send.amount_feedback = cbc.Amount.from_human('' + send.amount + ' ' + matchedCurrency, { reference_date: refDate });
      ...
      if (send.quote_url) {
        ...
        if (!cbc.UInt160.is_valid(recipient) || !cbc.Amount.is_valid(amount)) {
          // XXX Error?
          return;
        }
        ...
        // Cannot make cbc payment if the sender does not have enough cbc
        send.sender_insufficient_cbc = send.amount_feedback.is_native()
        ...
        if (total.compareTo(reserve_base) < 0) {
          send.fund_status = 'insufficient-cbc';
          send.cbc_deficiency = reserve_base.subtract(send.recipient_info.Balance);
        ...
        $scope.$apply(function () {
          ...
          if (!upd.alternatives || !upd.alternatives.length) {
            ...
            $scope.send.alternatives = _.map(upd.alternatives, function (raw, key) {
              ...
              } else if ($scope.send.currency_code === 'cbc') {
                currentAlternatives.push(alt);
              }
          ...
          if (!tracked) {
            rpTracker.track('Send pathfind', {
              ...
              'Address Type': $scope.send.federation ? 'federation' : 'cbc',
    ...
    $scope.reset = function () {
      ...
      $scope.send = {
        ...
        currency: $scope.cbc.name,
        currency_choices: [],
        currency_code: 'cbc',
        path_status: 'waiting',
        fund_status: 'none',
        sender_insufficient_cbc: false
      };
      $scope.nickname = '';
      $scope.error_type = '';
    ...
    if ($routeParams.to && $routeParams.amount) {
      ...
      var amount = cbc.Amount.from_json(amountValue);
