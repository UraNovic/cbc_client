    ...
    $scope.exportCsv = function() {
      ...
      for (var i = 0; i < $scope.historyShow.length; i++) {
        ...
        if (type === 'TrustSet') {
          ...
          lineTrust.TrustAddr = cbcName(transaction.counterparty);
          ...
          if (sent === null) {
            ...
            linePayment.ToAddr = cbcName(transaction.counterparty);
            linePayment.FromAddr = cbcName(id.account);
          }
          else {
            // If received, counterparty is Address From
            linePayment.FromAddr = cbcName(transaction.counterparty);
            linePayment.ToAddr = cbcName(id.account);
          }
          ...
          if (exists(transaction.amountSent)) {
            ...
            linePayment.SentCcy = exists(amtSent.currency) ? amtSent.currency : 'cbc';
            if (exists(transaction.sendMax)) linePayment.SentIssuer = cbcName(transaction.sendMax.issuer);
          }

          linePayment.RecvAmount = formatAmount(transaction.amount);
          linePayment.RecvCcy = transaction.currency;
          linePayment.RecvIssuer = cbcName(issuerToString(transaction.amount.issuer()));
